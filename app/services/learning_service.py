...
class LearningService:
    """Сохранение сообщения, обновление цепи Маркова и дедупликация при генерации."""
    ...
    async def is_duplicate(
        ...
        """True если текст дословно повторяет или начинается как сообщение из обучения.

        Для коротких текстов (< 3 токенов) — точное SQL-совпадение.
        Для остальных — проверка случайного префикса длиной 3, 4 или 5 токенов
        против in-memory кэша. Случайная длина добавляет вариативность: одно и то же
        сообщение может пройти фильтр на одной попытке и быть отклонено на другой.
        """
