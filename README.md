...
2. Заполните `BOT_TOKEN` (и `OWNER_ID`, если нужен контроль параметров во время работы через чат).
3. Запустите:
```bash
python main.py
```
...
- `/set <key> <value>` — изменение параметров во время работы (только `OWNER_ID`).
