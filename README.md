...
Число явных упоминаний и размер списка подписчиков ограничиваются
`PIVO_EXPLICIT_MENTIONS_LIMIT` и `PIVO_SUBSCRIBER_FANOUT_LIMIT`.

Аргументы влияют на тело приглашения: время встраивается в фразы про сбор, а
при указанном поводе бот собирает нейтральное тело сообщения без игровых
activity-фраз вроде `СИГейм`, `Codenames`, `рисовалка` и конфликтов с
ведущим. Если переданы только упоминания, используется обычный шаблон без
изменения контекста.
