...
- После каждого `/pivo` владелец (`OWNER_ID`) получает в личку короткий отчёт:
  название чата, сколько подписчиков упомянуто по `user_id` / строкой `@ник` /
  пропущено и сколько entity `text_mention` вернул сервер. Тот же агрегат идёт в
  лог; личка нужна потому, что решение по O2 принимает владелец, а логи ему
  недоступны. Выключается `PIVO_REPORT_TO_OWNER=false` (или `/set`); сбой
  доставки на работу `/pivo` не влияет.
