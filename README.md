...
Текущая версия использует **variable-order модель**:
- основной порядок: `n=3` (триграммы),
- fallback: `n=2`,
- fallback: `n=1` (если включен backoff).
...
2. Заполните `BOT_TOKEN` (и `OWNER_ID`, если нужен runtime-контроль через чат).
3. Запустите:
```bash
python main.py
```
...
- `ENABLE_BACKOFF` (`true`/`false`) — включить fallback на низшие порядки.
- `BACKOFF_MIN_ORDER` (`1`/`2`) — минимальный порядок fallback.
...
4. Обновляет статистику переходов:
   ...
   - `starts`: `(w1,w2)` — fallback-старты;
   - `transitions`: `(w1,w2)->w3`;
   - `transitions1`: `w1->w2`.

## Логика генерации
1. Старт берется из `starts3` (или из seed).
2. На каждом шаге:
...
- `/set <key> <value>` — runtime-изменение параметров (только `OWNER_ID`).
- `/setprob 0.2` — быстрый сеттер вероятности ответа (только `OWNER_ID`).
