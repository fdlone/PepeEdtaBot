...
- `/quirk_stats` — достижим ли в этом чате порог «завсегдатая», после которого
  бот начинает обращаться к человеку (L2): сколько людей имеют счётчик
  отвеченных обращений, максимум, сколько перешагнули `USER_QUIRK_MIN_INTERACTIONS`.
  Только числа — счётчики хранятся анонимно (HMAC), и диагностика этого не
  отменяет. Доступно только `OWNER_ID`.
...
- `/config` — основные действующие настройки чата; доступно всем участникам.
  `/config full` — полный список; доступно админам этого чата и `OWNER_ID`,
  то есть тому же кругу, который может эти значения менять через `/set`.
  Отказ отвечается явно: краткая форма вместо полной молча не подставляется.
- `/set <key> <value>` — изменить runtime-настройку **в этом чате** (`/set help`
  — подсказка по ключам). Формы: `/set global <key> <value>` меняет глобальное
  значение во всех чатах без своих переопределений, `/set reset <key>`
  возвращает чат к глобальному. Скоупленные формы доступны админам чата и
  `OWNER_ID`; **глобальная — только `OWNER_ID`** (см. O5 в `docs/CLOSED.md`:
  вектор закрыт скоупом, а не правами — админ чата и так им управляет).
- `/setprob 0.2` — быстрый setter вероятности ответа для этого чата
  (`/setprob global 0.2` — во всех). Права те же, что у `/set`.
...
Команды ограничены по частоте на пару «участник + чат»: дешёвые ответы из
памяти (`/help`, `/ping`, `/config`, `/pivo_privacy`) — 15 с, чтение из базы
(`/stats`) — 60 с, запись в базу (`/pivo_on`, `/pivo_off`) — 30 с, `/clear` —
час. Превышение по умолчанию игнорируется молча; там, где человек ждёт
подтверждения (`/clear`, `/pivo_on`, `/pivo_off`), бот один раз отвечает,
сколько осталось ждать. Без лимита намеренно оставлены owner-only команды
(`/set`, `/setprob`, `/pivo_check`) и `/pivo` — у него собственная суточная
квота, а троттлинг глушил бы вызов до хендлера, из-за чего человек вместо
«лимит исчерпан» получал тишину. Само объяснение «лимит исчерпан» при этом
дросселируется: не чаще раза в минуту на пару «участник + чат», дальше
повторные вызовы остаются без ответа — иначе отказ сам становился потоком
сообщений от бота.

Изменения через `/set` действуют только до перезапуска процесса. Переопределения
живут в памяти по чатам и вытесняются вместе с остальным состоянием чата (тот же
TTL и лимит, что у настроения и анти-повтора): чат, о котором бот давно не
слышал, возвращается к глобальным значениям. `/config` показывает эффективные
для чата значения и отмечает, какие из них переопределены.
...
используется текущий список подписчиков `/pivo_on`.

Два предела работают по-разному. Явных упоминаний нельзя указать больше
...
людей не позвана. Список подписчиков, наоборот, усекается до
`PIVO_SUBSCRIBER_FANOUT_LIMIT`, а в сообщение добавляется пометка, что упомянуты
не все: отказ ломал бы команду для всего чата навсегда — достаточно было
подписаться сверх предела, и вернуть `/pivo` мог только `/clear confirm`.

Повод и время режутся по длине (200 и 40 символов): иначе достаточно длинный
аргумент делал итоговое сообщение длиннее лимита Telegram, и отправка падала
при каждом вызове.
...
- `/pivo` работает строго по opt-in: подписки хранятся как HMAC-индексированные
  ...
  зашифрован Fernet'ом. Ключ Fernet выводится из `PIVO_ENCRYPTION_SECRET` через
  HKDF-SHA256 с доменной меткой `pivo:fernet` — так же, как ключ маскирования
  `chat_id` выводится с меткой `logging:chat_id`: отдельная метка означает
  отдельный ключ. Чтение идёт набором ключей (`MultiFernet`): актуальный
  шифрует, прежний (голый SHA-256 от секрета) только читает, и уже сохранённые
  строки переходят на новый ключ при очередной перезаписи профиля — без
  остановки бота и без единовременной миграции таблицы. Оба секрета обязаны
  быть не короче 32 символов, иначе бот не стартует: функция вывода ключа
  защищает ровно настолько, насколько непредсказуем сам секрет.
...
- Тот же инвариант действует и для артефактов репозитория: тестов, фикстур,
  документации, сохранённых результатов прогонов. Идентификаторы в них
  синтетические, а проверяет это `tests/test_no_real_chat_ids.py` — правило,
  которое держится только на внимательности, здесь уже один раз не удержалось.
  Новый синтетический идентификатор объявляется в списке этого теста.
