...
повод заменяет игровые/activity-упоминания вроде `СИГейм`, `Codenames`,
`рисовалка` и похожие блоки в выбранном шаблоне. Если переданы только
упоминания, используется обычный шаблон без изменения контекста.
