...
`/pivo`; смена `PIVO_HMAC_SECRET` дополнительно обнуляет статус «завсегдатаев»
(L2) и разрывает сопоставимость масок `chat_id` в логах через границу смены.
Ни то, ни другое не является аварией, но и не откатывается.

Схема вывода ключа шифрования (HKDF вместо голого SHA-256) сменилась
2026-08-06. Миграции данных не требуется: чтение идёт набором ключей, прежний
ключ остаётся читающим, а строки переходят на новый при очередной перезаписи
профиля подписчика (`/pivo_on` или первое сообщение участника за сутки).
