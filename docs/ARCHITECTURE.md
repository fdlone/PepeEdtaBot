...
Про вывод в `/config full` помнить **не нужно**: он порождается из реестра, и
новая ручка появляется там сама. Это не всегда было так — блок вёлся руками и
показывал 52 ключа из 97 при трёх независимых обещаниях «полный список»,
причём невидимыми оказались директор ответа, настроение, причуды и
`pivo_mention_by_id`, то есть ровно то, чем чат отличается от дефолта.
Расхождение было заложено этой самой инструкцией: она перечисляла реестр,
`Settings` и `RuntimeState` и молчала про вывод. Полноту теперь держит тест
против реестра, а не память автора.
...
- `RUN date -Is > /app/BUILD_AT` сразу после `COPY` рантайма — идентичность
  сборки, первая строка `/stats`. Время, а не ревизия: `.git` в контекст сборки
  не уходит, а требовать ревизию аргументом сборки значит требовать помнить о
  ней при каждом деплое. Место в файле выбрано под кэш слоя: штамп обновляется
  тогда и только тогда, когда изменилось содержимое `main.py` или `app/`.
  Читает штамп `app/presentation/bot_messages.py`, путём от пакета, поэтому вне
  контейнера строка честно говорит `unknown`.
...
**по именам** и проверка, что первая строка `/stats` отдаёт разбираемую дату
сборки. Белый список `COPY` иначе молча теряет файлы: сборка остаётся зелёной, а
бот падает при первом подключении к БД. По именам, а не по количеству, — потому
что замена файла один к одному количество сохраняет. Смок идёт штатным
