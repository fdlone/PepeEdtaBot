...
| SIM-3 | `lexical_diversity` и скорерный `repetition_penalty` меряют одно и то же | флипы argmax ~3% каждый; математически 1−diversity == доля повторов токенов | Слить в один компонент (PR #72), ранжирование не меняется, бейзлайны байт-идентичны |
| SIM-4 | `coherence_penalty` — шум после SIM-1 | флипы ~0.4%; остаток — 0.10 на ~2% кандидатов (order-2) | Удалить (PR #72) |
...
- PR #72 — слияние diversity→repetition, удаление coherence, схлопывание
  staleness-гейтов (stacked на #70).
- PR #73 — stem-матчинг контекстных состояний вместо prefix, включён по
  дефолту (stacked на #72).

### Changed files
- По веткам PR #70–#73: `app/core/markov.py`, `candidate_scorer.py`,
...
- `tests.test_eval_generation` — бейзлайны байт-стабильны на #72; на #70 и
