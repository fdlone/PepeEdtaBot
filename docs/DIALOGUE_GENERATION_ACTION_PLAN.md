...
| M2 reply-probability AI Director | **done** (2026-07-03), merged via PR #54 | `app/core/reply_policy.py` gains pure `conversation_momentum` (rate/mention/reply-thread blend → [0,1]), `burst_factor` (post-reply ×2 for `REPLY_BURST_BOOST_SEC`, then ×0.5 for `REPLY_BURST_SUPPRESS_SEC`), `effective_reply_probability` (momentum → `[REPLY_PROBABILITY_MIN, REPLY_PROBABILITY_MAX]` band × mood × burst), `within_hourly_cap` (`REPLY_MAX_PER_HOUR` safety gate). `should_reply_to_message` takes `hourly_cap_ok`. Handler decouples rhythm tracking from mood so the director's EWMA signals exist even with mood off; `RuntimeState.note_reply_sent` records reply timestamps for cap+burst. Master toggle `REPLY_DIRECTOR_ENABLED` (off → legacy flat `reply_probability`). Mentions always answered, never capped. Carried the Stage 1–3 review fixes (mood clamp, `long`-shadow, `_pick` caveat, typo, docs). **Review fix (PR #55):** `note_reply_sent` now takes `unprompted` — mention answers update cooldown/burst but are *not* appended to `recent_reply_times`, so a mentioned-heavy chat no longer exhausts the cap and silences the director; the burst factor uses a negative sentinel when a chat has no prior reply (instead of `last_ts=0.0` vs monotonic time, which faked a recent reply right after process start) |
| M3 emoji channel | **done** (2026-07-03) | new `app/core/emoji.py` (pure `extract_emojis`/`count_emojis`/`sample_emoji`/`append_emoji_flavor`), migration `011_chat_emoji_stats` + `ChatEmojiStatsRepo` keyed by raw `chat_id` (same contour as the word model, wiped in `clear_chat`). Learning handler records this chat's emoji frequencies from raw text before the word gates (captures emoji-only reactions); `ResponseGenerator` appends a frequency-sampled emoji (`count^0.5` flattening) with chance `EMOJI_APPEND_CHANCE`, suppressed after a "?" and boosted in heated mood. Stale rows decay (halve) at init via `decay_chat_emoji_stats`. Eval keeps the channel off. **Review fixes (PR #55):** `extract_emojis` excludes skin-tone modifiers (U+1F3FB–1F3FF) from the pictograph range and folds paired regional indicators into whole flags (lone half dropped), so a bare modifier/flag-half never lands in the stats or a reply; `normalize_reply_for_repeat` strips the appended emoji (via `strip_trailing_emojis`) before the anti-repeat compare, so an emoji flavor no longer lets an already-sent phrase slip past the S1 exact-match window |
