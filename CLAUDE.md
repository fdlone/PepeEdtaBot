...
- Маскировать `chat_id` везде, **включая текст чужих исключений**. С
  ...
  `aiogram` и проходит все записи, включая трассы `exc_info`. Правило четырежды
  держалось перечислением точек и четырежды промахивалось на новом месте —
  точечные обёртки вокруг `str(exc)` трассу не покрывают в принципе.
