...
| ловушка | прецедент |
|---|---|
| Новая ветка кандидата обязана идти через **полный** `_finalize_attempt` — хвостовой конвейер **и** четыре гейта формы, а не только `detokenize` | общий вход `finalize_candidate_tokens` `app/core/markov.py:755`, тело `_finalize_attempt` там же:2214; нарушитель — seeded, починен в M3R-101 (`app/core/response_generator.py:923`); map §3.4 |
| Новый пер-чатовый кэш обязан добавиться в `forget_chat`, иначе `/clear` его переживёт | `app/services/learning_service.py:563`, врезка — `_absorb_message` там же:620 |
...
| Новый канал данных обязан иметь счётчик «вернул пусто» — иначе канал, выключенный **данными**, неотличим от выключенного **ручкой** | map §3.2: `rank_seeds` при `n_docs=0` и пустой `get_hot` — две подсистемы молча мертвы в проде. Образец счётчика — M3R-141: `note_seed_ranking` / `note_hot_ngram_draw` / `note_context_dropped` (`app/core/generation_telemetry.py:317`, :329, :351), видны в `/stats` (в проде с рестарта 24.08, O7 закрыт 01.09). Знаменатель обязателен: `None` читается как «не спрашивали», измеренный ноль — как «спросили и ответил» |
| Новый хвостовой слой ответа обязан обновить `normalize_reply_for_repeat`, иначе анти-повтор не узнаёт отправленную форму | `app/core/response_generator.py:223`; map §4 |
