...
- Гейт, объявивший `requires_both_modes`, снимается в **двух режимах** `ctx` и
  ...
  (`ctx_generation_share`). Прод-доля на 2026-09-01 **не измерена** (O7,
  копится только после редеплоя) — до неё взвешенный вердикт недоступен.
  Флаг объявлен у `phase9_interp` и `structural_escape`; у `phase5_promotion`
  его нет — решение о добавлении за владельцем (пороги пре-регистрированы).
