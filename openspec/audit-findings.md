...
| B7 | B | `tests/test_log_masking.py`, `tests/test_error_handler.py`, `tests/test_migrator.py`, `tests/fixtures/legacy_real_schema.sql`, `docs/CLOSED.md`, `tools/sweeps/*_results.jsonl` | Реальный prod `chat_id` (тот же, что у основного чата в `db_prod_copy`; сам идентификатор здесь не приводится — см. `tests/test_no_real_chat_ids.py`) закоммичен в репозиторий — ~280 вхождений, из них 267 в отслеживаемых `tools/sweeps/*.jsonl`. Это прямо противоречит инварианту, ради которого написан `log_masking`: сырой идентификатор чата не должен утекать наружу. | medium | Заменить на синтетические id в тестах/доках, вычистить или разотслеживать sweep-результаты. | `privacy-scrub-prod-chat-id` |
...
## Ход выполнения (фаза 3)

Состояние на 2026-08-06, ветка `audit/pivo-security-and-privacy`.
Закрыто 10 из 13 изменений; каждое — отдельный коммит, каждое заархивировано
через `openspec archive`.

| # | change | коммит | статус |
|---|--------|--------|--------|
| 1 | `security-pivo-abuse-limits` | `dc30cbe` | закрыт |
| 2 | `privacy-scrub-prod-chat-id` | `741abb4` (+ `7ef783c`) | закрыт |
| 3 | `security-pivo-crypto` | `dd8d0ad` | закрыт |
| 4 | `restrict-config-full` | `900edd4` | закрыт |
| 5 | `perf-model-volume` | `30bd353` | закрыт |
| 6 | `perf-learning-caches` | `0f28b0e` | закрыт |
| 7 | `perf-generation-hotpath` | `ad9d4c7` | закрыт |
| 8 | `fix-runtime-overlay-tick` | `813bab3` | закрыт |
| 9 | `simplify-dead-code` | `a46aa35` | закрыт |
| 10 | `simplify-layering` | `c98e644` | закрыт |
| 11 | `simplify-db-facade` | — | **осталось** |
| 12 | `simplify-learning-handler` | — | **осталось** |
| 13 | `docs-refresh` | — | **осталось, выполняется последним** |

Замеры на копии прода (до → после):

- генерация ответа: 91.0 → 63.5 мс;
- чтение объёма модели (на каждое сообщение): 12.7 → 0.014 мс;
- выборка «горячих» n-грамм (почти на каждом ответе): 40.8 → 0.42 мс;
- кэши обучения после выученного сообщения: ~95 → 0.015 мс.

Тестов 878 → 919, покрытие 93% при пороге 87%. Хеш генерации
(`tools/generation_hash.py`, 400 генераций) не менялся ни разу:
`11ad32a2315287b6c0397dfa230e4cd227e8f9c62ba0f136fba27421cd9e8ad1`.

### Что знать перед продолжением

- **#11 механический, но крупный**: переводит потребителей с ~40 методов-делегатов
  `Database` на прямые обращения к репозиториям — заденет сервисы, хендлеры и
  заметную часть тестов. После #10 у ядра есть порт, и его состав менять не
  придётся.
- **#12 трогает горячий путь**: обязательный шаг 0 — зафиксировать хеш
  генерации и порядок обращений к генератору случайных чисел.
- **#13 выполняется последним**, иначе документация опишет промежуточное
  состояние.
- Активный change владельца `fix-pivo-mention-notifications` правит те же
  `app/handlers/pivo.py` и `app/services/pivo_service.py`, которые
  переписаны в #1 и #3 — конфликт при мерже ожидаем.
- Найденное по ходу, но не входившее в находки и **не закрытое**: сбой суточного
  затухания статистики роняет сохранение сообщения целиком
  (`decay_flavor_stats_if_due` вызывается из `save_message_and_update_model`).
  Требует решения, должен ли сбой обслуживания стоить сообщения.
