...
Состояние на 2026-08-07, ветка `audit/pivo-security-and-privacy`.
Закрыты все 13 изменений; каждое — отдельный коммит, каждое заархивировано
...
| 11 | `simplify-db-facade` | `65d2c0b` | закрыт |
| 12 | `simplify-learning-handler` | `2b86a8a` | закрыт |
| 13 | `docs-refresh` | `6008ef7` + докоммит | закрыт |
...
Тестов 878 → 941, покрытие 94% при пороге 87%. Хеш генерации
(`tools/generation_hash.py`, дефолтный прогон — 1000 генераций) не менялся ни
разу: `5a72e2d413abf235636dcc3cc99e29f50417b2e3cb947d7d73cc88e1e9eb664b`.

### Что знать после закрытия

- \*\*Документация переписана, а не дописана\*\* (#13): у каждого факта одно место.
  Значения ручек живут только в `.env.example` — он сверяется с реестром
  тестом, поэтому молча разъехаться не может; `docs/GENERATION_PIPELINE.md`
  называет ручки, но не повторяет их значения. README — витрина и навигация.
